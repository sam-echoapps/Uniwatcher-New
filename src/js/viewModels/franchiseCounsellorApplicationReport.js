    ...
    function (oj,ko,$, app, ArrayDataProvider, ojconverterutils_i18n_1, ListDataProviderView, ojdataprovider_1, PagingDataProviderView) {
        ...
        class ApplicationReport {
            constructor(args) {
                ...
                self.getCount = ()=>{
                    let franchiseId = sessionStorage.getItem("userFranchiseId")
                    self.data([])
                    $.ajax({
                        url: BaseURL+"/getFranchiseCounsellorApplicationCountYear",
                        type: 'POST',
                        data: JSON.stringify({
                ...
                self.getApplicationReport = ()=>{
                    ...
                    let franchiseId = sessionStorage.getItem("userFranchiseId")
                    let dataUrl = "/getFranchiseCounsellorApplicationCSDReport"
                    if(radio=="ASD"){
                        dataUrl = "/getFranchiseCounsellorApplicationASDReport"
                    }
                    $.ajax({
                        url: BaseURL+dataUrl,
                        type: 'POST',
                ...
                self.getApplicationCount = ()=>{
                    ...
                    let toDate = self.toDate();
                    let radio = self.selectRadio();
                    let dataUrl = "/getFranchiseCounsellorApplicationCSDCount"
                    let courseType = self.course();
                    courseType = courseType.join(",");
                    let institution = self.institution()
                    institution = institution.join(",");
                    let franchiseId = sessionStorage.getItem("userFranchiseId")
                    if(radio=="ASD"){
                        dataUrl = "/getFranchiseCounsellorApplicationASDCount"
                    }
                    $.ajax({
                        url: BaseURL+dataUrl,
                        type: 'POST',
