        ...
        class ApplicationReport {
            constructor(args) {
                ...
                self.getCount = ()=>{
                    ...
                    self.data([])
                    $.ajax({
                        url: BaseURL+"/getFranchiseApplicationCountYear",
                        type: 'POST',
                        data: JSON.stringify({
                ...
                self.getApplicationReport = ()=>{
                    ...
                    let franchiseId = sessionStorage.getItem("userFranchiseId")
                    let dataUrl = "/getFranchiseApplicationCSDReport"
                    if(radio=="ASD"){
                        dataUrl = "/getFranchiseApplicationASDReport"
                    }
                    $.ajax({
                        url: BaseURL+dataUrl,
                        type: 'POST',
                ...
                self.getApplicationCount = ()=>{
                    ...
                    let toDate = self.toDate();
                    let radio = self.selectRadio();
                    let dataUrl = "/getFranchiseApplicationCSDCount"
                    let courseType = self.course();
                    courseType = courseType.join(",");
                    let institution = self.institution()
                    institution = institution.join(",");
                    let franchiseId = sessionStorage.getItem("userFranchiseId")
                    if(radio=="ASD"){
                        dataUrl = "/getFranchiseApplicationASDCount"
                    }
                    $.ajax({
                        url: BaseURL+dataUrl,
                        type: 'POST',
