define(['ojs/ojcore',"knockout","jquery","appController", "ojs/ojarraydataprovider", "ojs/ojconverterutils-i18n",
    ...
    function (oj,ko,$, app, ArrayDataProvider, ojconverterutils_i18n_1, ListDataProviderView, ojdataprovider_1) {
        ...
        class FranchiseStudentManagerReport {
            constructor(args) {
                var self = this;
                ...
                self.statisticCount = ()=>{
                    ...
                    let franchise = self.franchiseId();
                    let status = self.selectList();
                    status = status.join(",");
                        $.ajax({
                            url: BaseURL+"/getFranchiseCounsellorStaticCount",
                            type: 'POST',
                            data: JSON.stringify({
                                fromDate: fromDate,
                                toDate: toDate,
                ...
                self.showData = ()=>{
                    ...
                    $.ajax({
                        url: BaseURL+"/getFranchiseCounsellorStudentManagerEDReport",
                        type: 'POST',
                        data: JSON.stringify({
                            fromDate: fromDate,
                            toDate: toDate,
        ...
        return  FranchiseStudentManagerReport;
    }
);