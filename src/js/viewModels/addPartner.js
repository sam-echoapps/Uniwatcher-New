    ...
    function (oj,ko,$, app, ArrayDataProvider, ListDataProviderView, ojdataprovider_1, ojconverterutils_i18n_1) {
        ...
        class Partners {
            constructor(args) {
                ...
                self.partnerData = ko.observableArray([]);
                self.partnersCount = ko.observable();

                self.getPartnersDetails = ()=>{
                    self.partnerData([]);
                    $.ajax({
                        url: BaseURL+"/getPartnersDetails",
                        type: 'GET',
                        ...
                        success: function (data) {
                            if(data[0] != "No data found"){
                                ...
                                let len = data.length;
                                self.partnersCount(len)
                                for(let i=0;i<len;i++){
                                    self.partnerData.push({
                                        partnerId: data[i][0],
                ...
                self.partnerDataProvider = ko.computed(function () {
                    ...
                    const arrayDataProvider = new ArrayDataProvider(self.partnerData, { keyAttributes: 'DepartmentId' });
                ...
                self.addPartner = ()=>{
                    document.getElementById("addPartner").open();
                }
                self.cancelAddPartnerPopup = ()=>{
                    document.getElementById("addPartner").close();
                }
                ...
                self.viewPartner = (e)=>{
                    let partnerId = e.currentTarget.id;
                    sessionStorage.setItem("partnerId", partnerId);
                    window.location.href = `/?ojr=partnerProfile`;
                }
